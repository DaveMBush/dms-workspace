...
- [ ] Task 1: Read Story 64.1 Dev Agent Record and confirm root cause (AC: #1, #2)
  - [ ] Subtask 1.1: Read the Dev Agent Record from Story 64.1 to get the confirmed root cause and the exact code path involved
  - [ ] Subtask 1.2: Re-read `global-universe.component.ts` `filteredData$` computed — confirm whether `excludeLoadingRows` filter is the Round 5 regression path
  - [ ] Subtask 1.3: Re-read `enrich-universe-with-risk-groups.function.ts` — confirm the Epic 60 placeholder fix is still in place and that placeholders have `symbol: ''`

- [ ] Task 2: Evaluate fix strategy options and choose the minimal-risk approach (AC: #1, #2)
  - [ ] Subtask 2.1: Option A — Remove `excludeLoadingRows` from `filteredData$` and instead give `buildPlaceholderUniverseEntry` a non-empty sentinel symbol value (e.g., `'\u200B'` ZERO WIDTH SPACE) that renders blank in the table but bypasses the `symbol !== ''` check; update the template to hide cells with that sentinel if needed
  - [ ] Subtask 2.2: Option B — Remove `excludeLoadingRows` from `filteredData$` entirely and instead handle blank-row suppression at the template level using `*ngIf="row.symbol"` on the row or cell content so CDK sees the full array but the user never sees empty text
  - [ ] Subtask 2.3: Option C — Add an `isPlaceholder` boolean field to `EnrichedUniverse`; set it `true` in `buildPlaceholderUniverseEntry`; replace `symbol !== ''` with `!row.isPlaceholder` in `filteredData$` — but this preserves array length only if the filter is removed entirely; otherwise still shrinks
  - [ ] Subtask 2.4: Choose and document the selected approach — **the preferred approach is to keep the full stable-length array for CDK and handle blank display differently, not by filtering the data array**

- [ ] Task 3: Apply the fix (AC: #1, #2, #3)
  - [ ] Subtask 3.1: Modify `filteredData$` in `global-universe.component.ts` to keep placeholder rows in the array (do not filter out `symbol === ''` rows from the CDK data)
  - [ ] Subtask 3.2: Ensure the table template does not display placeholder row content visibly to the user — this typically means the `dms-base-table` renders the row at the correct position but shows blank cells, which is acceptable during the brief `isLoading` window
  - [ ] Subtask 3.3: Add an explanatory code comment citing the regression history: Epics 29, 31, 44, 60, and 64; explain WHY removing the filter preserves CDK scroll stability
  - [ ] Subtask 3.4: Update `enrich-universe-with-risk-groups.function.spec.ts` if any test assertions change; do NOT remove assertions — update them to reflect the new behavior

- [ ] Task 4: Verify with Playwright MCP server (AC: #3)
  - [ ] Subtask 4.1: Navigate to the Universe screen; fast-scroll to the bottom; confirm no blank rows in the MCP screenshot
  - [ ] Subtask 4.2: Click the Symbol sort header; wait for network idle; fast-scroll to the bottom; confirm no blank rows
  - [ ] Subtask 4.3: Document the verification result in the Dev Agent Record

- [ ] Task 5: Confirm the Story 64.1 E2E test passes (AC: #4, #5)
  - [ ] Subtask 5.1: Run `pnpm run e2e:dms-material:chromium` — confirm the previously-failing test at `universe-scrolling-regression.spec.ts:200` now passes
  - [ ] Subtask 5.2: Run `pnpm all` — confirm no regressions
