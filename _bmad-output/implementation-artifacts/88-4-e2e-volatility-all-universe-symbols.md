...
- [x] Task 1: Inspect existing volatility e2e infrastructure to decide where the new test lives (AC: #4)

  - [x] Open `apps/dms-material-e2e/src/volatility-visibility.spec.ts` (Epic 84) and
        `apps/dms-material-e2e/src/volatility-new-categories.spec.ts` (Epic 84) to understand patterns
  - [x] Open `apps/dms-material-e2e/src/helpers/seed-volatility-column-base.helper.ts`,
  ...
  - [x] Decide: extend `volatility-visibility.spec.ts` with a new `describe('Story 88.4 — held + unheld symbols', ...)` block, OR create a new file `apps/dms-material-e2e/src/volatility-all-universe-symbols.spec.ts`. Pick one and document the choice in Dev Notes.

- [x] Task 2: Create or extend a seed helper for the held + unheld pair (AC: #1, #2)

  - [x] Create `apps/dms-material-e2e/src/helpers/seed-volatility-held-and-unheld.helper.ts` (or extend an existing helper if the patterns line up cleanly)
  - [x] The helper must seed:
  ...
  - [x] The helper must trigger the universe-sync flow (or whichever flow Story 88.3 wires) so that volatility columns get populated. If the e2e env mocks `fetchDividendHistory`, the helper must arrange that mock to return a deterministic non-empty payload for the unheld symbol.
  - [x] Return both symbol names so the test can use them
  - [x] Document in Dev Notes how the seed helper handles the `dividendhistory.net` dependency in the e2e environment (real network call vs. mocked vs. test seed endpoint)

- [x] Task 3: Write the failing E2E test **first** (TDD) (AC: #1, #2, #3)

  - [x] In the chosen spec file, add a `describe('Story 88.4 — Volatility for held and unheld universe symbols', ...)` block
  - [x] Inside, add `test('Vol column shows a real category for both held and unheld symbols', async ({ page }) => { ... })`:
  ...
  - [x] Add `test('SVol column shows a real category for both held and unheld symbols', async ({ page }) => { ... })`:
...
const volCell = page.locator('tr.mat-mdc-row').filter({ hasText: symbolName }).locator('td').first(); // Vol column is first column per Epic 81; verify per current template
...
- Task 1 decision: Created new file `volatility-all-universe-symbols.spec.ts` (not extending the existing spec) to keep Story 88.4 tests clearly scoped and independent.
- Task 2 dividendhistory.net approach: Used option (a) — directly write `volatility_long` / `volatility_short` to a valid category in the seed helper via Prisma. This verifies the **rendering** half of Epic 88 (Universe screen displays stored values). It does NOT verify the computation wiring introduced by Stories 88.2+88.3. The rendering test is the correct scope for this story; the wiring is covered by the unit tests in Stories 88.2 and 88.3.
- SVol `aria-label` confirmed as `Short-Term Volatility: <category>` (verified from `global-universe.component.html`).
- No `waitForTimeout` or `waitForLoadState('networkidle')` used; `expect.poll(...)` used for retried icon assertions.
- Tasks 4-7 (E2E run verification, flake check, browser verification, full regression) are deferred to Phase 3 as instructed.

### File List

- `apps/dms-material-e2e/src/helpers/seed-volatility-held-and-unheld.helper.ts` (created)
- `apps/dms-material-e2e/src/volatility-all-universe-symbols.spec.ts` (created)

### Change Log

- 2026-04-29: Created seed helper and spec file for Story 88.4 TDD tests (Tasks 1-3).
