...
for (let i = 0; i < args.length; i++) {
  if (args[i] === "--stdout") stdoutFile = args[++i];
    ...
    console.error("Usage: validate-epics-and-stories.mjs (--stdout <file> | --stdin) [--root <dir>]");
    process.exit(2);
  }
}
...
const storyFiles = candidates.filter((p) =>
  /_bmad-output\/implementation-artifacts\/stories\/epic-\d+\/\d+\.\d+-.+\.md$/.test(p),
);
...
for (const [dir, stories] of epicDirs) {
  ...
  if (numbered.length % 3 !== 0 && numbered.length % 2 !== 0)
    fail(`${dir}: ${numbered.length} stories is not a multiple of 2 or 3 (triple ordering broken)`);

  const firstStory = stories
    .filter((s) => s.path.match(new RegExp(dir + "/")))
    .sort((a, b) => a.path.localeCompare(b.path))[0];
  if (firstStory && !/^Status:\s*skip\s*$/m.test(firstStory.content))
    fail(`${dir}: first story is not status "skip" (expected unit-test story first)`);
  else pass(`${dir}: first story is status "skip"`);
}
...
if (epicsFiles.length > 0 && existsSync(resolve(projectRoot, epicsFiles[0]))) {
  const epicsContent = readFileSync(resolve(projectRoot, epicsFiles[0]), "utf8");
  const epicsFound = [...epicsContent.matchAll(/^## Epic (\d+):/gm)].map((m) => m[1]);
  for (const dir of epicDirs.keys()) {
    const epicNum = dir.replace("epic-", "");
    if (!epicsFound.includes(epicNum))
      fail(`epics.md has no "## Epic ${epicNum}:" entry but story dir ${dir} exists`);
