...
echo "🔍 Gathering ECS service information..."
# Generate ECS service information
if aws ecs list-clusters --query 'clusterArns' --output text | grep -q 'dms'; then
    CLUSTER_NAME=$(aws ecs list-clusters --query 'clusterArns' --output text | grep 'dms' | head -n1 | xargs basename)
    ...
    if [ -n "$CLUSTER_NAME" ]; then
        ...
        echo "No DMS clusters found" > docs/operations/current-services.txt
    fi
else
    echo "No ECS clusters found" > docs/operations/current-services.txt
fi

echo "🗄️ Gathering RDS information..."
...
echo "⚖️ Gathering Load Balancer information..."
...
echo "☁️ Gathering CloudFront information..."
...
echo "🪣 Gathering S3 bucket information..."
...
echo "✅ Service documentation updated successfully"
echo "📁 Files updated:"
