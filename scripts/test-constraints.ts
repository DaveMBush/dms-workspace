...
async function testConstraints() {
  console.log('🧪 Testing Story E3 schema integrity and performance constraints...\n');
  ...
  try {
    ...
    try {
      ...
      console.log('❌ Universe symbol uniqueness constraint failed - duplicates allowed');
    } catch (error) {
    ...
    try {
      ...
      console.log('❌ Risk group name uniqueness constraint failed - duplicates allowed');
    } catch (error) {
    ...
    console.log('Universe indexes found:', universeIndexes.map(i => i.name));
    ...
    console.log('Screener indexes found:', screenerIndexes.map(i => i.name));
