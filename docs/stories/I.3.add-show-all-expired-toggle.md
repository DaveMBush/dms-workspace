...
<div class="filter-control" *ngIf="hasExpiredSymbols()">
  <p-checkbox 
    binary="true"
    [(ngModel)]="showAllExpired"
    (onChange)="onShowAllExpiredChange($event)"
    inputId="showAllExpired"
    [disabled]="isLoading()"
  />
  <label for="showAllExpired" class="filter-label">
    Show All Expired
    <i class="pi pi-info-circle" 
       pTooltip="When checked, shows all expired symbols regardless of position status. Unchecked shows only expired symbols with open positions."
       tooltipPosition="top">
    </i>
  </label>
</div>
