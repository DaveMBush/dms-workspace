...
This document outlines the automated processes and maintenance procedures for keeping the RMS documentation up-to-date and accurate.

## Overview

The RMS documentation system includes several automated processes to ensure documentation remains current, accurate, and easily maintainable across all environments and team changes.
...
aws ecs describe-services \
  ...
  --services $(aws ecs list-services --cluster rms-production --query 'serviceArns[]' --output text) \
...
curl -s https://api.rms.company.com/health | jq '.' > docs/operations/api-health-schema.json 2>/dev/null || echo '{"status": "unreachable"}' > docs/operations/api-health-schema.json
...
# send_notification "⚠️  Stale documentation detected - review required" "#rms-team"
```
