...
3. `.opencode/agents//commit-and-pr.agent.md`
4. `.opencode/agents//code-rabbit.agent.md`
5. `.opencode/agents//quality-validation.agent.md`
...
2. Commit and PR creation:
   - If no open PR exists, call the `runSubagent` tool now with the following parameters for commit and PR creation:
     ...
     - `prompt`: Read the full contents of `.opencode/agents//commit-and-pr.agent.md` and include them verbatim, substituting `${story}` with the actual story ID.
...
3. CodeRabbit loop:
   - Call the `runSubagent` tool now with the following parameters for the full CodeRabbit loop:
     ...
     - `prompt`: Read the full contents of `.opencode/agents//code-rabbit.agent.md` and include them verbatim, substituting `${story}` with the actual story ID.
