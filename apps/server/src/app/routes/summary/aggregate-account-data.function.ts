...
function calculateDeposits(
  divDeposits: Array<{ universeId: string | null; amount: number }>
): number {
  return divDeposits
    .filter(function filterDeposits(dd) {
...
function calculateDividends(
  ...
  return divDeposits
    .filter(function filterDividends(dd) {
      return dd.universeId !== null;
    })
...
export function aggregateAccountData(
  accounts: AccountWithData[] | AccountWithTradesAndDeposits[]
): AggregatedData {
  ...
  const capitalGains = accounts.reduce(function aggregateCapitalGains(
    acc,
    account
  ) {
