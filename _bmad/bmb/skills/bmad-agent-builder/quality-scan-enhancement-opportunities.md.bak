...
**Don't force it.** Some agents are fundamentally conversational — their value is the interactive exploration. Flag those as "fundamentally interactive" and move on. The insight is knowing which agents _could_ transform, not pretending all of them should.
...
4. **Stay in your lane.** Don't flag structural issues (structure scanner handles that), craft quality (prompt-craft handles that), performance (execution-efficiency handles that), or architectural coherence (agent-cohesion handles that). Your findings should be things _only a creative thinker would notice_.
...
- Did I actually _inhabit_ different user archetypes (including the automator), or did I just analyze from the builder's perspective?
- Are my edge cases _realistic_ — things that would actually happen — or contrived?
...
- Is every suggestion _actionable_ — could someone implement it from my description?
